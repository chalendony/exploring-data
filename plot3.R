...
png(filename = "plot3.png",
   width = 480,
   height = 480)
plot(x = as.POSIXlt(tab$Timefmt, format = "%d"),
     y = as.vector(tab$Sub_metering_1),type = "l",
     ylab = "Energy sub metering",
     xlab = "")
lines(as.POSIXlt(tab$Timefmt, format = "%d"), as.vector(tab$Sub_metering_2), col="red")
lines(as.POSIXlt(tab$Timefmt, format = "%d"), as.vector(tab$Sub_metering_3), col="blue")
